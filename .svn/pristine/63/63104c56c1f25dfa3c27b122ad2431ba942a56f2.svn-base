		...
		$(document).ready(function() {
			...
			$.ajax({
				...
				success: function(data){
					//console.log(data);
					var html = "<select name='spTypeCode' id='spTypeCode' class='required input-xlarge'>";
		...
		function setShopNameAndCode(){
			...
			$.ajax({
				...
				success: function(data){
					...
					$("#spTypeCode").empty();
					for(var i=0;i<data.length;i++){
						$("#spTypeCode").append("<option value='"+data[i].spTypeCode+"'>"+data[i].spTypeName+"</option>");  //添加一项option
						/*$("#spTypeCode").prepend("<option value='-1'>请选择</option>"); //在前面插入一项option*/
					}
		...
		<div class="control-group">
			...
			<div class="controls" id="spTypeCodeSelectId">
				...
				<form:select path="spTypeCode" class="required input-xlarge">
					<%--<form:option value="-1">请选择</form:option>--%>
					<form:options items="${spTypeByCodeList}" itemLabel="spTypeName" itemValue="spTypeCode" htmlEscape="false"/>
				</form:select>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
